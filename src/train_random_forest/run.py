...
def go(args):
    ...
    logger.info(f"X_train size: {X_train.shape[0]} rows, {X_train.shape[1]} columns")
    logger.info(f"X_val size: {X_val.shape[0]} rows, {X_val.shape[1]} columns")
    logger.info(f"y_train size: {y_train.shape[0]} rows")
    logger.info(f"y_val size: {y_val.shape[0]} rows")

    logger.info("Preparing sklearn pipeline")

    sk_pipe, processed_features = get_inference_pipeline(rf_config, args.max_tfidf_features)

    rf_model = sk_pipe.named_steps['random_forest']
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    reshape_to_1d = FunctionTransformer(lambda x: x.ravel())
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train a Random Forest model and export it to W&B as an MLFlow model")
